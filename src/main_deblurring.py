...
if __name__ == '__main__':
    ...
    for img in GT_imgs:
        ...
        if img != "comic.png" and img != "lenna.png":
          continue
        ...
        I_DIP, network, z, psnr_history, ssim_history, psnr_bm3d, ssim_bm3d, blurred_img,loss = deblurring.dip_deblur(
            img, noise_lvl, filter_type, loss_type, directory)

        final_psnr = compare_psnr(I[[2, 1, 0], :, :], I_DIP)

        blurred_img = np.clip(blurred_img.cpu().numpy() * 255, 0, 255).astype(np.uint8)
        print(blurred_img.shape)
        blurred_img = blurred_img.squeeze().transpose(1, 2, 0)  
        cv.imwrite(f'./results/images/blurred_{img}', cv.cvtColor(blurred_img, cv.COLOR_RGB2BGR))

        constructed_img = np.clip(I_DIP * 255, 0, 255).astype(np.uint8)
        constructed_img = constructed_img.transpose(1, 2, 0)
        cv.imwrite(f'./results/images/reconstructed_{loss}_{img}', cv.cvtColor(constructed_img, cv.COLOR_RGB2BGR))
