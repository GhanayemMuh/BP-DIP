...
def torch_fourier_conv(f, k):
    ...
    for i in range(3):
        fft_x = fft_im = torch.view_as_real(torch.fft.fft2(f[:, i:i + 1, :, :])).to(self.device)
        fft_kernel = torch.view_as_real(torch.fft.fft2(expanded_kernel_shift)).to(self.device)
...
def mae(input, target):
    return torch.mean(torch.abs(input - target)).to(self.device)

def ssim_loss(input, target):
    #input_np = input.detach().cpu().numpy()
    #target_np = target.detach().cpu().numpy()
    ssim_index = compare_ssim(input, target, multichannel=True, win_size=3, data_range=1.0)
    return torch.tensor(1 - ssim_index, requires_grad=True).float().to(self.device)

def psnr_loss(pred, target):
    mse_loss = F.mse_loss(pred, target)
    max_pixel_value = 1.0  # Assuming images are normalized between 0 and 1
    psnr = 10 * torch.log10(max_pixel_value**2 / mse_loss)
    return -psnr
...
def BP_loss(x_hat, y, sigma, filter_type):
    ...
    fft_y = fft_im = torch.view_as_real(torch.fft.fft2(y)).cpu()
...
def dip_deblur(img_name, noise_lvl, filter_type, loss_type, directory):
    ...
    psf = np.squeeze(psf)
    y_tv = y.cpu().numpy()
    y_tv = np.moveaxis(y_tv, 1, -1)
    y_tv = np.squeeze(y_tv)
    img_tv = denoise_tv_chambolle(y_tv, weight=0.1)
    img_tv = np.moveaxis(img_tv, -1, 0)
    psnr_tv = compare_psnr(imgs['HR_np'], img_tv)
    ssim_tv = compare_ssim(np.moveaxis(imgs['HR_np'], 0, -1), np.moveaxis(img_tv, 0, -1), multichannel=True, win_size=3, data_range=1.0)
    ...
    def closure():
        ...
        if loss_type == 'dip':
            ...
            fft_y = torch.view_as_real(torch.fft.fft2(y)).cpu()
            if loss == 'mse':
              total_loss = mse(fourier_conv, fft_y).to(self.device)
            elif loss == 'mae':
              total_loss = mae(fourier_conv, fft_y).to(self.device)
            elif loss == 'ssim':
              total_loss = ssim_loss(np.moveaxis(torch_to_np(x_hat), 0, -1), np.moveaxis(imgs['HR_np'], 0, -1)).to(self.device)
            elif loss == 'psnr':
              total_loss = psnr_loss(x_hat.squeeze().to(self.device), torch.tensor(imgs['HR_np']).squeeze().to(self.device)).to(self.device)
        elif loss_type == 'bp':
        ...
        ssim = compare_ssim(np.moveaxis(orig_img, 0, -1), np.moveaxis(x_hat_np, 0, -1), multichannel=True, win_size=3, data_range=1.0)
    ...
    return constructed_img, net, net_input, psnr_history_short, ssim_history_short, psnr_tv, ssim_tv, y,loss